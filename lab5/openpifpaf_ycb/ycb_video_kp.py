...
class YCBVideoKp(DataModule):
    """
    DataModule for the YCB-Video dataset.
    """

    train_annotations = 'C:/Users/Adam - Student/Documents/AGH/DS/Semestr_8/uczenie-maszyn/lab5/coco_train.json'
    val_annotations = 'C:/Users/Adam - Student/Documents/AGH/DS/Semestr_8/uczenie-maszyn/lab5/coco_test.json'
    eval_annotations = val_annotations
    train_image_dir = 'C:/Users/Adam - Student/Documents/AGH/DS/Semestr_8/uczenie-maszyn/YCB-Video_data/data/0010'
    val_image_dir = 'C:/Users/Adam - Student/Documents/AGH/DS/Semestr_8/uczenie-maszyn/YCB-Video_data/data/0010'
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        group = parser.add_argument_group('data module YCB Video')

        group.add_argument('--ycb_video-train-annotations',
                           default=cls.train_annotations)
        group.add_argument('--ycb_video-val-annotations',
                           default=cls.val_annotations)
        group.add_argument('--ycb_video-train-image-dir',
                           default=cls.train_image_dir)
        group.add_argument('--ycb_video-val-image-dir',
                           default=cls.val_image_dir)

        group.add_argument('--ycb_video-square-edge',
                           default=cls.square_edge, type=int,
                           help='square edge of input images')
        # Fix missing options
        group.add_argument('--ycb_video-augmentation',
                           default=True, type=bool,
                           help='augmentation of training data')
        group.add_argument('--ycb_video_eval_annotation_filter',
                           default=True, type=bool,
                           help='')
        group.add_argument('--ycb_video_apply_local_centrality',
                           default=True, type=bool,
                           help='')

        assert not cls.extended_scale
        group.add_argument('--ycb_video-extended-scale',
                           default=False, action='store_true',
                           help='augment with an extended scale range')
        group.add_argument('--ycb_video-orientation-invariant',
                           default=cls.orientation_invariant, type=float,
                           help='augment with random orientations')
        group.add_argument('--ycb_video-blur',
                           default=cls.blur, type=float,
                           help='augment with blur')
        assert cls.augmentation
        group.add_argument('--ycb_video-no-augmentation',
                           dest='apollo_augmentation',
                           default=True, action='store_false',
                           help='do not apply data augmentation')
        group.add_argument('--ycb_video-rescale-images',
                           default=cls.rescale_images, type=float,
                           help='overall rescale factor for images')
        group.add_argument('--ycb_video-upsample',
                           default=cls.upsample_stride, type=int,
                           help='head upsample stride')
        group.add_argument('--ycb_video-min-kp-anns',
                           default=cls.min_kp_anns, type=int,
                           help='filter images with fewer keypoint annotations')
        group.add_argument('--ycb_video-bmin',
                           default=cls.b_min, type=int,
                           help='b minimum in pixels')
        group.add_argument('--ycb_video-apply-local-centrality-weights',
        ...
        group.add_argument('--ycb_video-no-eval-annotation-filter',
                           dest='apollo_eval_annotation_filter',
                           default=True, action='store_false')
        group.add_argument('--ycb_video-eval-long-edge', default=cls.eval_long_edge, type=int,
                           help='set to zero to deactivate rescaling')
        assert not cls.eval_extended_scale
        group.add_argument('--ycb_video-eval-extended-scale',
                           default=False, action='store_true')
        group.add_argument('--ycb_video-eval-orientation-invariant',
                           default=cls.eval_orientation_invariant, type=float)
        group.add_argument('--ycb_video-use-24-kps', default=False, action='store_true',
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        # Apollo specific
        cls.train_annotations = args.ycb_video_train_annotations
        cls.val_annotations = args.ycb_video_val_annotations
        cls.eval_annotations = cls.val_annotations
        cls.train_image_dir = args.ycb_video_train_image_dir
        cls.val_image_dir = args.ycb_video_val_image_dir
        cls.eval_image_dir = cls.val_image_dir

        cls.square_edge = args.ycb_video_square_edge
        cls.extended_scale = args.ycb_video_extended_scale
        cls.orientation_invariant = args.ycb_video_orientation_invariant
        cls.blur = args.ycb_video_blur
        cls.augmentation = args.ycb_video_augmentation  # loaded by the dest name
        cls.rescale_images = args.ycb_video_rescale_images
        cls.upsample_stride = args.ycb_video_upsample
        cls.min_kp_anns = args.ycb_video_min_kp_anns
        cls.b_min = args.ycb_video_bmin
        if args.ycb_video_use_24_kps:
        ...
        # evaluation
        cls.eval_annotation_filter = args.ycb_video_eval_annotation_filter
        cls.eval_long_edge = args.ycb_video_eval_long_edge
        cls.eval_orientation_invariant = args.ycb_video_eval_orientation_invariant
        cls.eval_extended_scale = args.ycb_video_eval_extended_scale
        if args.ycb_video_apply_local_centrality:
            if args.ycb_video_use_24_kps:
                raise Exception(
                    "Applying local centrality weights only works with 66 kps.")
    ...
    def _preprocess(self):
        ...
        return transforms.Compose([
            ...
            transforms.RandomApply(transforms.HFlip(
                self.CAR_KEYPOINTS, self.HFLIP), 0.5),
            rescale_t,
        ...
        if cls.eval_extended_scale:
            ...
            rescale_t = [
                transforms.DeterministicEqualChoice([
                    ...
                    transforms.RescaleAbsolute(
                        (cls.eval_long_edge - 1) // 2 + 1),
                ], salt=1)
            ]
